...
def train(hyp):
    ...
    t_weights = opt.t_weights # 老师模型权重
    ...
    if t_cfg:
        ...
        elif t_weights.endwith('.weights'):
            load_darknet_weights(t_model, t_weights)

        else:
            raise Exception('Unsupported weight format! Please use .pt or .weights')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            soft_target = 0
            reg_ratio = 0  #表示对于目标回归效果没有老师模型好，针对这部分此时再与ground truth学习

            if t_cfg:
                if mixed_precision:
                    with torch.no_grad():
                        t_output = t_model(imgs)
                else:
                    _, t_output = t_model(imgs)
                soft_target = distillation_loss1(pred, t_output, nc, imgs.size(0))
                #soft_target, reg_ratio = distillation_loss2(model, targets, pred, t_output)
                loss += soft_target
            ...
            s = ('%10s' * 2 + '%10.3g' * 7) % ('%g/%g' % (epoch, epochs - 1), mem, *mloss, msoft_target, len(targets), img_size)
