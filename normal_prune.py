...
class opt():
    model_def = "cfg/ghost32-yolov3-visdrone.cfg"
    data_config = "data/visdrone.data"
    model = 'weights/best/best.pt'
...
eval_model = lambda model:test(model=model, imgsz=640, cfg='cfg/ghost-yolov3-visdrone.cfg', data='data/visdrone.data')
...
with torch.no_grad():
    ...
    #results, maps = test.test(cfg=opt.model_def,
                              #data=opt.data_config,
                              #batch_size=4,
                              #img_sz=608,
                              #model=ema.ema,
                              #conf_thres=0.001
...
#compact_model = Darknet([model.hyp.copy()] + compact_module_defs).to(device)
for i, mdef in enumerate(compact_module_defs):
    if mdef['type'] == 'shortcut':
        mdef['from'] = str(mdef['from'][0])
    if mdef['type'] == 'route':
        if len(mdef['layers']) == 2 :
            mdef['layers'] = str(mdef['layers'][0]) + ',' + str(mdef['layers'][1])
        else:
            mdef['layers'] = str(mdef['layers'][0])
    if mdef['type'] == 'yolo':
        mdef['mask'] = str(mdef['mask'][0]) + ',' + str(mdef['mask'][1]) + ',' + str(mdef['mask'][2])
        mdef['anchors'] = '4,5,  6,10,  14,9,  11,18,  25,15,  21,30,  47,26,  37,53,  87,65'

pruned_cfg_file = write_cfg('pruned.cfg',  [model.hyper.copy()] + compact_module_defs)

compact_model = Darknet('pruned.cfg').to(device)
print(compact_model)
...
compact_model_name = 'weights/yolov3_visdrone_normal_pruning_'+str(percent)+'percent.weights'
