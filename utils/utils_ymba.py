...
def build_targets(p, targets, model):
    ...
    for i, j in enumerate(model.yolo_layers):
        # anchor_vec是下采样后的anchor大小
        anchors = model.module.module_list[j].anchor_vec if multi_gpu else model.module_list[j].anchor_vec
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain  # 这里为了得到下采样倍数
...
def soft_nms(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=False):
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=False, classes=None, agnostic=False):
    """
...
def distillation_loss1(s_output, t_output, num_classes, batch_size):
    T = 3.0
    Lambda_ST = 0.01
    criterion_st = torch.nn.KLDivLoss('reduction=sum')
    s_output = torch.cat([i.view(-1, num_classes + 5) for i in s_output])
    t_output = torch.cat([i.view(-1, num_classes + 5) for i in t_output])
    loss_st = criterion_st(nn.functional.log_softmax(s_output/T, dim=1), nn.functional.softmax(t_output/T, dim=1))* (T*T) / batch_size
